...
import {
  fetchGitHubActivitiesWithSession,
  type GitHubActivity,
  type Repository,
} from '@/lib/github';
...
export function ActivityFeed({ selectedRepository }: ActivityFeedProps) {
  ...
  const loadGitHubData = async () => {
    ...
    try {
      ...
      if (errorMessage.includes('GitHub account not connected')) {
        setError(
          'Please connect your GitHub account to see real activity data.'
        );
      } else {
  ...
  const allActivities = [...githubActivities, ...mockActivities].sort(
    (a, b) => {
      ...
      <CardHeader>
        <div className='flex items-center justify-between'>
          <div>
            ...
            <CardDescription>
              Real-time updates from Notion, Slack, and GitHub
            </CardDescription>
          </div>
            ...
            <div key={activity.id} className='flex items-start gap-4'>
              ...
              <div className='flex-1 space-y-1'>
                <div className='flex items-center gap-2'>
                  ...
                  <Badge
                    variant='outline'
                    className={`${getColor(activity.type)} flex items-center gap-1 px-1.5 py-0`}
                  >
